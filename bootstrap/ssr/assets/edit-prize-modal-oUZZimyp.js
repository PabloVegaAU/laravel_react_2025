  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (data.level_required_id) {
        ...
        formData.append("required_level_id", String(data.level_required_id));
      } else {
      ...
      if (!response.ok) {
        if (responseData.errors) {
          Object.values(responseData.errors).flat().forEach((errorMsg) => toast.error(String(errorMsg)));
          return;
        }
      ...
      toast.error((error == null ? void 0 : error.message) || "Error al actualizar el premio");
    } finally {
      setIsLoading(false);
    }
  };
