...
export default function ProgressBar() {
  ...
  const bgColorClass = progressColorClass.includes('red')
    ? 'bg-red-50 border-red-300'
    : progressColorClass.includes('orange')
      ? 'bg-orange-50 border-orange-300'
      : progressColorClass.includes('yellow')
        ? 'bg-yellow-50 border-yellow-300'
        : progressColorClass.includes('blue')
          ? 'bg-blue-50 border-blue-300'
          : 'bg-green-50 border-green-300'
  const textColorClass = progressColorClass.includes('red')
    ? 'text-red-700'
    : progressColorClass.includes('orange')
      ? 'text-orange-700'
      : progressColorClass.includes('yellow')
        ...
        : progressColorClass.includes('blue')
          ? 'text-blue-700'
          : 'text-green-700'
  const badgeBgClass = progressColorClass.includes('red')
    ...
    <div className='flex-1'>
      <div className={`rounded-xl border p-4 shadow-sm ${bgColorClass}`}>
        ...
        <div className='mb-2 flex items-center space-x-2'>
          ...
          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${badgeBgClass}`}>{progress.progress_percent}% completado</span>
        </div>
