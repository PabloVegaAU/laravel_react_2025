...
export default function CreateAvatar({ auth }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    post(route('teacher.avatars.store'), {
      onSuccess: () => {
        toast.success('Avatar creado exitosamente')
      },
      onError: () => {
        toast.error('Error al crear el avatar')
      },
      forceFormData: true
    })
  }
  ...
  return (
    <AppLayout user={auth.user}>
      <Head title='Crear Nuevo Avatar' />

      <div className='container mx-auto py-6'>
        <div className='mb-6 flex items-center justify-between'>
          <h2 className='text-2xl font-bold tracking-tight'>Crear Nuevo Avatar</h2>
        </div>

        <Card className='mx-auto max-w-2xl'>
          <CardHeader>
            <CardTitle>Información del Avatar</CardTitle>
            <CardDescription>Complete los detalles del nuevo avatar.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className='space-y-6'>
              ...
              <div className='flex justify-end space-x-4 pt-4'>
                ...
                <Button type='submit' disabled={processing}>
                  {processing ? 'Guardando...' : 'Guardar Avatar'}
                </Button>
              </div>
            </form>
          </CardContent>
